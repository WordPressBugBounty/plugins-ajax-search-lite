...
/* harmony export */   recursiveCheckVisibility: function() { return /* binding */ recursiveCheckVisibility; },
/* harmony export */   scrollToFirstVisibleElement: function() { return /* binding */ scrollToFirstVisibleElement; },
...
const recursiveCheckVisibility = function(element) {
  if (typeof element.checkVisibility === "undefined") {
    return true;
  }
  let el = element, visible = true;
  while (el !== null) {
...
const scrollToFirstVisibleElement = function(elements, offset = 0) {
  for (const element of elements) {
    if (recursiveCheckVisibility(element)) {
      window.scrollTo({
        top: element.getBoundingClientRect().top - 120 + window.pageYOffset + offset,
        behavior: "smooth"
      });
      return true;
    }
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   buildRestUrl: function() { return /* binding */ buildRestUrl; },
...
/* harmony export */   parse_str: function() { return /* binding */ parse_str; }
/* harmony export */ });
...
function md5(str) {
  ...
  view.setUint32(paddedLen - 8, len << 3 >>> 0, true);
  view.setUint32(paddedLen - 4, len >>> 29, true);
  ...
  for (let i = 0; i < paddedLen; i += 64) {
    const w = [];
    for (let t = 0; t < 16; t++) w[t] = view.getUint32(i + t * 4, true);
    ...
    for (let j = 0; j < 64; j++) {
      ...
      const rot = a + f + k[j] + w[g] >>> 0;
      const temp = d;
      d = c;
      c = b;
      b = b + (rot << s[j] | rot >>> 32 - s[j]) >>> 0;
      a = temp;
    }
    a = a + aa >>> 0;
    b = b + bb >>> 0;
  ...
  const toHexLe = (n) => [n & 255, n >>> 8 & 255, n >>> 16 & 255, n >>> 24 & 255].map((byte) => byte.toString(16).padStart(2, "0")).join("");
  return toHexLe(a) + toHexLe(b) + toHexLe(c) + toHexLe(d);
}
function parse_str(str) {
  const decodeQC = (v) => decodeURIComponent(v.replace(/\+/g, "%20"));
  const splitKeys = (rawKey) => {
    const keys = [];
    let start = 0;
    for (let i = 0; i < rawKey.length; i++) {
      if (rawKey[i] === "[" && !start) {
        start = i + 1;
      } else if (rawKey[i] === "]" && start) {
        if (!keys.length) keys.push(rawKey.slice(0, start - 1));
        keys.push(rawKey.substring(start, i));
        start = 0;
        if (rawKey[i + 1] !== "[") break;
      }
    }
    return keys.length ? keys : [rawKey];
  };
  const normRoot = (key) => {
    let k = key;
    for (let i = 0; i < k.length; i++) {
      if (k[i] === " " || k[i] === "." || k[i] === "[") {
        k = k.slice(0, i) + "_" + k.slice(i + 1);
      }
      if (k[i] === "[") break;
    }
    return k;
  ...
  const nextNumKey = (t) => {
    let max = -1;
    for (const p in t) {
      if (/^\d+$/.test(p) && Number(p) > max) max = Number(p);
    }
    return String(max + 1);
  };
  const isSeqArr = (o) => {
    ...
    const nums = keys.filter((k) => /^\d+$/.test(k)).map(Number).sort((a, b) => a - b);
    if (nums.length !== keys.length) return false;
  ...
  const toArrays = (o) => {
    ...
    if (isSeqArr(o)) {
      const arr = [];
      Object.keys(o).map(Number).sort((a, b) => a - b).forEach((k) => {
        arr[k] = toArrays(o[String(k)]);
      });
      return arr;
    }
    const res = {};
    for (const k in o) {
      if (Object.prototype.hasOwnProperty.call(o, k)) res[k] = toArrays(o[k]);
    }
    return res;
  };
  const result = {};
  for (const seg of String(str).replace(/^&/, "").replace(/&$/, "").split("&")) {
    const [rk, ...rvp] = seg.split("=");
    let rawKey = decodeQC(rk ?? "").trimStart();
    const value = rvp.length === 0 ? "" : decodeQC(rvp.join("="));
    const nulIdx = rawKey.indexOf("\0");
    if (nulIdx > -1) rawKey = rawKey.slice(0, nulIdx);
    if (!rawKey || rawKey[0] === "[") continue;
    const keys = splitKeys(rawKey);
    keys[0] = normRoot(keys[0]);
    let cur = result, par = result, curKey = "";
    for (let i = 0; i < keys.length; i++) {
      curKey = keys[i].replace(/^['"]/, "").replace(/['"]$/, "");
      par = cur;
      if ((curKey === "" || curKey === " ") && i !== 0) curKey = nextNumKey(cur);
      if (Object(cur[curKey]) !== cur[curKey]) cur[curKey] = {};
      cur = cur[curKey];
    }
    par[curKey] = value;
  }
...
function buildRestUrl(base, path, params = {}) {
  const url = base + path;
...
window.ASL = { ...window.ASL, ...{
  ...
  api: api,
  initialized: false,
  initCache: function(force = false) {
    if (this.initialized && !force) {
      return;
    }
    const ASL = window.ASL;
    ASL.cache.list = /* @__PURE__ */ new Set();
    ASL.cache.timestamp = (/* @__PURE__ */ new Date()).getTime();
    if (!ASL.cache.enabled || ASL.cache.type !== "super_file") {
      return;
    }
    fetch((0,other.buildRestUrl)(ASL.rest_url, "ajax-search-lite/results_cache/keyset", { t: ASL.cache.timestamp }), {
      method: "GET"
    }).then((r) => r.json()).then((r) => {
      ASL.cache.list = new Set(r);
    }).catch((e) => {
      console.log(e);
    });
  },
  ...
  initializeHighlight: function() {
    ...
    for (const data of window.ASL.highlight.data) {
      let selector = data.selector !== "" && domini_default()(data.selector).length > 0 ? data.selector : "article", phrase;
      ...
      if (data.scroll) {
        if (!(0,browser.scrollToFirstVisibleElement)(domini_default()(".asl_single_highlighted_exact").get(), data.scroll_offset)) {
          (0,browser.scrollToFirstVisibleElement)(domini_default()(".asl_single_highlighted").get(), data.scroll_offset);
        }
      }
    }
  },
...
/* unused harmony import specifier */ var filters_AslPlugin;



"use strict";
AslPlugin.prototype.setFilterStateInput = function(timeout) {
  let $this = this;
...
/* harmony default export */ var filters = ((/* unused pure expression or super */ null && (filters_AslPlugin)));
...
AslPlugin.prototype.gaGetTrackingID = function() {
  let ret = false;
...
AslPlugin.prototype.getLiveLoadAltSelectors = function() {
  return [
    ".search-content",
...
AslPlugin.prototype.usingLiveLoader = function() {
  const $this = this;
...
AslPlugin.prototype.liveLoad = function(selector, url, updateLocation, forceAjax) {
...
AslPlugin.prototype.getCurrentLiveURL = function() {
  let $this = this;
...
"use strict";
AslPlugin.prototype.showLoader = function() {
  this.n("proloading").css({
    display: "block"
  });
};
AslPlugin.prototype.hideLoader = function() {
  let $this = this;
  $this.n("proloading").css({
    display: "none"
  });
...
AslPlugin.prototype.loadASLFonts = function() {
...
AslPlugin.prototype.destroy = function() {
  let $this = this;
...
/* harmony default export */ var actions_other = ((/* unused pure expression or super */ null && (other_AslPlugin)));
...
AslPlugin.prototype.isRedirectToFirstResult = function() {
...
AslPlugin.prototype.doRedirectToFirstResult = function() {
  let _loc, url = "";
  if (this.ktype == "click") {
...
AslPlugin.prototype.doRedirectToResults = function(ktype) {
  let _loc;
  if (ktype == "click") {
    _loc = this.o.trigger.click_location;
  } else {
...
AslPlugin.prototype.getRedirectURL = function(ktype = "enter") {
...
AslPlugin.prototype.showResults = function() {
  let $this = this;
  $this.initResults();
...
AslPlugin.prototype.hideResults = function(blur = true) {
  let $this = this;
...
AslPlugin.prototype.showResultsBox = function() {
  let $this = this;
...
AslPlugin.prototype.addHighlightString = function($items) {
...
AslPlugin.prototype.scrollToResults = function() {
...
function registerLiveSearchStatistics(phrase, aspId, deviceType, foundResults, userId = 0, cached = 0) {
  ...
  _timer = setTimeout(() => {
    const items = [..._queue];
    _queue.length = 0;
    fetch((0,other.buildRestUrl)(statistics_ASL.rest_url, "ajax-search-lite/statistics/searches/add"), {
...
AslPlugin.prototype.searchAbort = function() {
  let $this = this;
  if ($this.post != null) {
    $this.post.abort();
  }
};
AslPlugin.prototype.searchWithCheck = function(timeout = 50) {
  let $this = this;
...
AslPlugin.prototype.search = function() {
  let $this = this;
...
{}
  ...
  if (domini_default()(".asl_es_" + $this.o.id).length > 0) {
    ...
    const doAjaxSearch = () => {
      $this.post = domini_default().fn.ajax({
        ...
        "success": function(r) {
          ...
          htmlContent = hooks_filters.Hooks.applyFilters("asl/search/html", htmlContent);
          $this.n("resdrg").html("");
    ...
    if (window.ASL.cache.enabled && window.ASL.cache.type === "super_file") {
      const _opts = (0,other.parse_str)(data.options);
      const _hash = (0,other.md5)(0 + $this.n("text").val() + JSON.stringify(_opts) + "0").slice(0, 14);
      if (window.ASL.cache.list.has(_hash)) {
        fetch(window.ASL.cache.url + _hash + ".json?t=" + search_ASL.cache.timestamp).then((response) => {
          ...
          const htmlContent = hooks_filters.Hooks.applyFilters("asl/search/html", resp.html ?? "");
          $this.n("resdrg").html("");
          $this.n("resdrg").html(htmlContent);
          $this.nodes.items = $this.n("resultsDiv").find(".item");
          $this.addHighlightString();
...
AslPlugin.prototype.searchFor = function(phrase) {
...
AslPlugin.prototype.toggleSettings = function(state) {
...
AslPlugin.prototype.closeResults = function(clear) {
...
AslPlugin.prototype.getStateURL = function() {
  let url, sep;
...
AslPlugin.prototype.filtersInitial = function() {
  return this.n("searchsettings").find("input[name=filters_initial]").val() == 1;
};
AslPlugin.prototype.filtersChanged = function() {
  return this.n("searchsettings").find("input[name=filters_changed]").val() == 1;
};
/* harmony default export */ var etc_api = ((/* unused pure expression or super */ null && (api_AslPlugin)));
...
AslPlugin.prototype.detectAndFixFixedPositioning = function() {
...
AslPlugin.prototype.fixResultsPosition = function(ignoreVisibility = false) {
...
AslPlugin.prototype.fixSettingsPosition = function(ignoreVisibility = false) {
...
AslPlugin.prototype.fixSettingsWidth = function() {
};
AslPlugin.prototype.hideOnInvisibleBox = function() {
  let $this = this;
...
AslPlugin.prototype.initMagnifierEvents = function() {
  let $this = this, t;
...
AslPlugin.prototype.initInputEvents = function() {
...
AslPlugin.prototype._initFocusInput = function() {
  let $this = this;
...
AslPlugin.prototype._initSearchInput = function() {
  let $this = this;
...
AslPlugin.prototype._initEnterEvent = function() {
...
AslPlugin.prototype._initFormEvent = function() {
  let $this = this;
...
AslPlugin.prototype.initNavigationEvents = function() {
  let $this = this;
...
AslPlugin.prototype.initOtherEvents = function() {
...
AslPlugin.prototype.orientationChange = function() {
  let $this = this;
  $this.detectAndFixFixedPositioning();
  $this.fixSettingsPosition();
  $this.fixResultsPosition();
};
AslPlugin.prototype.resize = function() {
  let $this = this;
  $this.detectAndFixFixedPositioning();
  $this.fixSettingsPosition();
  $this.fixResultsPosition();
};
AslPlugin.prototype.scrolling = function(ignoreVisibility) {
  let $this = this;
  $this.detectAndFixFixedPositioning();
...
AslPlugin.prototype.initResultsEvents = function() {
  let $this = this;
  $this.n("resultsDiv").css({
    opacity: "0"
  });
...
AslPlugin.prototype.monitorTouchMove = function() {
  let $this = this;
  $this.dragging = false;
...
AslPlugin.prototype.init = function(options, elem) {
...
AslPlugin.prototype.n = function(k) {
...
AslPlugin.prototype.initNodeVariables = function() {
  let $this = this;
...
AslPlugin.prototype.initEvents = function() {
...
AslPlugin.prototype.initResults = function() {
...
AslPlugin.prototype.initResultsBox = function() {
  let $this = this;
...
AslPlugin.prototype.initResultsAnimations = function() {
  this.n("resultsDiv").css({
...
"use strict";
AslPlugin.prototype.autocompleteGoogleOnly = function() {
...
AslPlugin.prototype.fixAutocompleteScrollLeft = function() {
...
/* harmony default export */ var autocomplete = ((/* unused pure expression or super */ null && (autocomplete_AslPlugin)));

;// ./src/client/plugin/core/events/autocomplete.ts
/* unused harmony import specifier */ var events_autocomplete_AslPlugin;



AslPlugin.prototype.initAutocompleteEvent = function() {
  let $this = this;
...
/* harmony default export */ var events_autocomplete = ((/* unused pure expression or super */ null && (events_autocomplete_AslPlugin)));
...
/* unused harmony import specifier */ var results_vertical_AslPlugin;



AslPlugin.prototype.showVerticalResults = function() {
  let $this = this;
  $this.showResultsBox();
  if ($this.n("items").length > 0) {
...
/* harmony default export */ var results_vertical = ((/* unused pure expression or super */ null && (results_vertical_AslPlugin)));
...
AslPlugin.prototype.showSettings = function() {
  let $this = this;
  $this.initSettings?.();
...
AslPlugin.prototype.hideSettings = function() {
  let $this = this;
  $this.initSettings?.();
...
/* harmony default export */ var settings = ((/* unused pure expression or super */ null && (settings_AslPlugin)));

;// ./src/client/plugin/core/events/facet.ts



AslPlugin.prototype.initFacetEvents = function() {
  let $this = this;
...
/* unused harmony import specifier */ var events_settings_AslPlugin;




AslPlugin.prototype.initSettingsSwitchEvents = function() {
  let $this = this;
...
AslPlugin.prototype.initSettingsEvents = function() {
  let $this = this, t;
...
/* harmony default export */ var events_settings = ((/* unused pure expression or super */ null && (events_settings_AslPlugin)));
...
AslPlugin.prototype.initSettings = function() {
...
AslPlugin.prototype.initSettingsBox = function() {
  let $this = this;
...
AslPlugin.prototype.initSettingsAnimations = function() {
  let $this = this;
